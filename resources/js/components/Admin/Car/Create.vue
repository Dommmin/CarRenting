<template>
    <div class="">
        <form @submit.prevent="storeCar(car)">
            <!-- Name -->
            <div>
            ...
            <!-- Buttons -->
            <div class="mt-4">
                <button :disabled="isLoading" class="inline-flex items-center px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl disabled:opacity-75 disabled:cursor-not-allowed">
...
const onFileChange = (e) => {
    // car.value.images = e.target.files;
    car.value.images = e.target.files[0];
};
...
const { storeCar, isLoading, validationErrors } = useCars();
